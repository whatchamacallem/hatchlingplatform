...
// Registers a function.  Use in a global scope.  E.g. "hxConsoleCommand(srand);"
#define hxConsoleCommand(x) hxConsoleConstructor HX_CONCATENATE(g_hxConsoleSymbol_,x)(hxCommandFactory(&(x)), HX_QUOTE(x))

// Registers a named function.  Use in a global scope a global variable.  Provided name must be a valid C identifier.
...
// Registers a variable.  Use in a global scope.  E.g. "bool g_isEnabled=false; hxConsoleVariable(g_isEnabled);"
#define hxConsoleVariable(x) hxConsoleConstructor HX_CONCATENATE(g_hxConsoleSymbol_,x)(hxVariableFactory(&(x)), HX_QUOTE(x))

// Registers a named variable.  Use in a global scope.  Provided name must be a valid C identifier.
...
// Evaluates a console command which either calls a function or sets a variable. E.g.: "srand 77" or "aVariable 5"
bool hxConsoleExecLine(const char* command);

// Executes a configuration file which is opened for reading.  Ignores blank lines and
