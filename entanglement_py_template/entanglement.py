...
def _verbose(verbose_level: int, x: str) -> None:
	'''
...
def _raise_error(c: Cursor, message: str) -> NoReturn:
	'''
...
def _get_dunder_name(cursor: Cursor) -> str:
	...
	if cursor.kind is CursorKind.CONSTRUCTOR: # type: ignore
		...
		if not name in _operator_name_map:
			_raise_error(cursor, f'{name} unsupported.')
...
def _calculate_python_package_path(cursor: Cursor) -> str:
	'''
...
def _get_base_class(cursor: Cursor) -> str:
	...
	for child in cursor.get_children():
		if child.kind == CursorKind.CXX_BASE_SPECIFIER: # type: ignore
			return _calculate_python_package_path(child.referenced)
	return '_Ctypes.Structure'

def _has_vtable(cursor: Cursor):
	'''
...
def _calculate_type_string(cursor: Cursor, cpp_type_ref: Type, symbols: Dict[str, List[Cursor]], result_kind: type_string_kind) -> str:
	...
	if cpp_type_canonical.kind in ( TypeKind.POINTER,		  # type: ignore
		...
		if pointee_type_canonical.kind in ( TypeKind.POINTER,		  # type: ignore
											TypeKind.LVALUEREFERENCE,  # type: ignore
											TypeKind.RVALUEREFERENCE): # type: ignore
			_raise_error(cursor, 'Only one pointer or reference allowed in an API type.')
			...
			if result_kind is type_string_kind.return_hint:
				if pointee_type_canonical.kind in _clang_to_ctypes_ptr_return:
					if cpp_type_canonical.kind in ( TypeKind.LVALUEREFERENCE,  # type: ignore
						...
						_raise_error(cursor, "Return char and wchar_t by reference unsupported.")
	...
	if definition_cursor.kind in (CursorKind.ENUM_DECL, CursorKind.STRUCT_DECL, CursorKind.CLASS_DECL): # type: ignore
		...
		if not py_name in symbols:
			_raise_error(definition_cursor, f'Missing definition for: {py_name}. Use ENTANGLEMENT_T.')
...
def _emit_python_api_doc(tabs: str, cursor: Cursor) -> List[str]:
	'''
...
def _emit_ctypes_function_args(cursor: Cursor, symbols: Dict[str, List[Cursor]], overloaded: bool) -> str:
	'''
...
def _emit_python_api_overload_arg0_isinstance(cursor: Cursor, symbols: Dict[str, List[Cursor]], overloaded: bool) -> str:
	'''
...
def _emit_python_api_function(namespace_tabs: str, cursor: Cursor, symbols: Dict[str, List[Cursor]], overloaded: bool) -> List[str]:
	...
	lines += _emit_python_api_doc(function_tabs, cursor)
	if overloaded:
		...
		# args.
		arg_str = _emit_ctypes_function_args(cursor, symbols, overloaded)
...
def _emit_python_api_overload_selector(namespace_tabs: str, overloads: List[Cursor], symbols: Dict[str, List[Cursor]]) -> List[str]:
	'''
	...
	lines: List[str] = []
	static_method = False
	if any(_is_staticmethod(cursor) for cursor in overloads):
		lines.append(namespace_tabs + '@_Staticmethod')
		static_method = True

	function_name = _get_dunder_name(overloads[0])
	...
	for arg_count, overload_group in sorted(arg_count_map.items()):
		...
		if len(inheritance_generation_map[0]) > 1:
			_raise_error(inheritance_generation_map[0][0],
		...
		for generation, generation_group in sorted(inheritance_generation_map.items(), reverse=True):
			for cursor in generation_group:
				internal_name = _get_internal_name(cursor)
				arg_str = _emit_ctypes_function_args(cursor, symbols, True)
				...
				if counter > 1:
					...
					arg0_selector = _emit_python_api_overload_arg0_isinstance(cursor, symbols, True)
...
def _emit_python_api_enum(namespace_tabs: str, cursor: Cursor) -> List[str]:
	'''
	...
	lines += _emit_python_api_doc(enum_tabs, cursor)
	...
	for constant in cursor.get_children():
		if constant.kind == CursorKind.ENUM_CONSTANT_DECL: # type: ignore
			...
			is_empty = False
	if is_empty:
		_raise_error(cursor, 'Empty enums not allowed in Python: ' + enum_name)
...
def _emit_python_api_class(namespace_tabs: str, cursor: Cursor) -> List[str]:
	'''
...
def _emit_python_api(symbols: Dict[str, List[Cursor]], sorted_symbols: List[List[Cursor]], api: List[str]) -> None:
	...
	for cursor_list in sorted_symbols:
		...
		cursor_namespace = _calculate_namespace(cursor0)
		...
		_verbose(2, f'emit_python_api {_calculate_python_package_path(cursor0)} {cursor0.kind.name}')
		...
		for cursor_list in symbols_at_depth:
			...
			if cursor0.kind in (CursorKind.CLASS_DECL, CursorKind.STRUCT_DECL): # type: ignore
				...
				if not field_count:
					...
					# classes.
					_raise_error(cursor0, 'Empty class/struct unsupported due to ctypes.')
...
def _emit_symbol_table(symbols: Dict[str, List[Cursor]], sorted_symbols: List[List[Cursor]], symbol_table: List[str]) -> None:
	'''
	...
	for cursor_list in sorted_symbols:
		if cursor_list[0].kind in ( CursorKind.FUNCTION_DECL, # type: ignore
			...
			for cursor in cursor_list:
				...
				# format them
				internal_name = _get_internal_name(cursor)
				symbol_name = _get_cxx_symbol_name(cursor)
...
def _symbols_add(cursor: Cursor, symbols: Dict[str, List[Cursor]]) -> None:
	...
	if not cursor.is_anonymous():
		if (cursor.spelling.startswith('_')
			...
			_raise_error(cursor, '1 leading underscore followed by a capital letter reserved by entanglement.py.')
		if cursor.spelling in keyword.kwlist or cursor.spelling in keyword.softkwlist:
			_raise_error(cursor, cursor.spelling + ' is a keyword in Python.')
		if cursor.spelling in ctypes_reserved:
			_raise_error(cursor, cursor.spelling + ' is reserved by ctypes.Structure.')

	sym : str = _calculate_python_package_path(cursor)
...
def _symbols_sort(symbols: Dict[str, List[Cursor]], sorted_symbols: List[List[Cursor]]) -> None:
	...
	for cursor_list in symbols.values():
		cursor0 = cursor_list[0]
		sort_key = _calculate_namespace(cursor0)
		sort_key.append(_get_dunder_name(cursor0))
...
def _symbols_gather_required(cursor: Cursor, symbols: Dict[str, List[Cursor]]) -> None:
	...
	if cursor.kind in (	CursorKind.TRANSLATION_UNIT, # type: ignore
	...
	elif not _is_annotated_entanglement(cursor):
		...
		_raise_error(cursor, f'{cursor.displayname} Templates are not supported.')
		...
		if cursor.is_definition() and cursor.access_specifier in (
			...
			_symbols_add(cursor, symbols)
			return
		else:
			_raise_error(cursor, f'{_get_name(cursor)} Enum is incomplete or private.')
		...
		if (cursor.linkage is LinkageKind.EXTERNAL # type: ignore
				...
				and not _is_arg_va_list(cursor)):
			_symbols_add(cursor, symbols)
			return
		else:
			_raise_error(cursor, f'{cursor.displayname} Functions must be extern and not variadic.')
		...
		if (cursor.is_definition() and not cursor.is_anonymous() and cursor.access_specifier in (
			...
			_raise_error(cursor, f'{_get_name(cursor)} Structs and classes must be complete, not be anonymous and be public.')
		...
		if (cursor.access_specifier is AccessSpecifier.PUBLIC # type: ignore
			...
			_raise_error(cursor, f'{cursor.displayname} Methods must be public, extern and not variadic.')

	else:
		_raise_error(cursor, f'Unsupported {cursor.kind.name} {_get_name(cursor)}')

	# fallthrough to children.
	for c in cursor.get_children():
		_symbols_gather_required(c, symbols)
...
def _assemble_output_file(python_api: List[str], structure_list: List[str], symbol_table: List[str]) -> List[str]:
	'''
...
def main(argv: List[str]) -> None:
	...
	_verbose(1, 'version:        ' + _version)
	_verbose(1, 'compiler_flags: ' + ' '.join(_arg_compiler_flags))
	...
	for header_file in _arg_header_files:
		_verbose(1, 'load_translation_unit ' + header_file)
		translation_unit = _load_translation_unit(header_file)
		_symbols_gather_required(translation_unit.cursor, symbols)
	...
	_symbols_sort(symbols, sorted_symbols)

	# Build the script sections.
	python_api: List[str] = []
	structure_list: List[str] = []
	symbol_table: List[str] = []
	_emit_python_api(symbols, sorted_symbols, python_api)
	_emit_structure_list(symbols, sorted_symbols, structure_list)
	_emit_symbol_table(symbols, sorted_symbols, symbol_table)

	# Assemble them into an output file.
	output_lines: List[str] = _assemble_output_file(python_api, structure_list, symbol_table)

	# Write output.
	with open(_arg_output_file, 'w') as f:
		f.write('\n'.join(output_lines))
		_verbose(1, f'status_message Wrote {len(output_lines)} lines to {_arg_output_file}.')
		return # success
