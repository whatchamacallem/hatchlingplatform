...
struct hxUniquePtr {
public:
	HX_INLINE hxUniquePtr() { m_ptr = hx_null; }
	HX_INLINE explicit hxUniquePtr(T* t) { m_ptr = t; }
	// Lacking an r-value in C++98, proper move semantics for temporaries are
	// not available.
	...
	HX_INLINE void reset(T* ptr = hx_null) {
		if (m_ptr && m_ptr != ptr) {
			Deleter d; d(m_ptr);
		}
		m_ptr = ptr;
	...
	HX_INLINE T* operator->() const { return m_ptr; }
	HX_INLINE operator bool() const { return m_ptr != hx_null; }
