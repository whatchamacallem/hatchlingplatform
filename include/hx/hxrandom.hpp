...
public:
	...
	constexpr hxrandom(uint64_t stream_ = 1u) : m_state_(stream_) { }
	...
	constexpr hxrandom& operator()(void) { return *this; }
	...
	constexpr operator float(void) {
		return (float)this->advance32() * (1.0f / 4294967296.0f); // 0x1p-32f
	}
	constexpr operator double(void) {
		return (double)this->advance64() * (1.0 / 18446744073709551616.0); // 0x1p-64;
	}
	constexpr operator uint8_t(void) {
		return this->advance32();
	}
	constexpr operator uint16_t(void) {
		return this->advance32();
	}
	constexpr operator uint32_t(void) {
		return this->advance32();
	}
	constexpr operator uint64_t(void) {
	...
	template<typename T_> constexpr T_ range(T_ base_, T_ size_) {
	...
	/// Returns [0..2^32).
	constexpr uint32_t advance32(void) {
	...
	/// Returns [0..2^64).
	constexpr uint64_t advance64(void) {
...
/// - `b` : A `hxrandom`.
template <typename T_> constexpr  T_ operator&(T_ a_, hxrandom& b_) {
...
/// - `b` : A `hxrandom`.
constexpr  int64_t operator&(int64_t a_, hxrandom& b_) {
   return (int64_t)((uint64_t)a_ & b_.advance64());
}
constexpr  uint64_t operator&(uint64_t a_, hxrandom& b_) {
...
template <typename T_> constexpr  T_ operator&(hxrandom& a_, T_ b_) {
	return b_ & a_;
}
...
template <typename T_> constexpr  T_ operator%(hxrandom& dividend_, T_ divisor_) {
