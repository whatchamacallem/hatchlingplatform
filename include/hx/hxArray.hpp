...
public:
	...
	// if Capacity is 0.
	HX_CONSTEXPR_FN explicit hxArray() { m_end = this->getStorage(); }
	...
	HX_CONSTEXPR_FN explicit hxArray(const hxArray& rhs_) : hxAllocator<T, Capacity_>() {
	...
	HX_CONSTEXPR_FN void assign(const T2_(&a_)[Sz_]) { assign(a_ + 0, a_ + Sz_); }
