...
#if HX_PROFILE
// Initiates a DMA transfer from src to dst of bytes length.  labelStringLiteral, if non-null,
// is used in profiling and debug diagnostic messages.
#define hxDmaStart(dst, src, bytes, labelStringLiteral) hxDmaStartLabeled(dst, src, bytes, labelStringLiteral)

// Waits until all DMA proceeding the corresponding call to hxDmaAddSyncPoint() is completed.
// labelStringLiteral, if non-null, is used in a profiler record generated by this event
// and in profiling and debug diagnostic messages.
#define hxDmaAwaitSyncPoint(syncPoint, labelStringLiteral) hxDmaAwaitSyncPointLabeled(syncPoint, labelStringLiteral)

// Waits until all DMA is completed.  labelStringLiteral, if non-null, is used in a profiler
