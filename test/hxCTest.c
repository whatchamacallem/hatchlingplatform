...
bool hxCTestSwap() {
	...
	return a[0] == 7 && a[1] == 3 && b[0].x == 70 && b[1].x == 30;
}

bool hxCTestMemory() {
	// try triggering a memory sanitizer
	void* b33 = hxMallocExt(33, hxMemoryManagerId_TemporaryStack, 16);

	char* t = hxStringDuplicate("_est", hxMemoryManagerId_TemporaryStack);
	t[0] = 't';
